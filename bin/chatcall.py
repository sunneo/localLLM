...
3. 如果只是打招呼，請文字回答。"""
        ...
        print(f"[*] {'[接力中]' if is_continuation else '[規劃中]'} 分析任務...", flush=True)
            ...
            for tool_name in self.available_tools:
                if tool_name in raw_res:
                    ...
                    plan_data = {
                        "theme": "自動復原任務",
                        "tags": suggested,
                        "tasks": [{"tool": tool_name, "params": {"task_description": user_input}}]
                    }
                    is_tool_call = True
                    break
        ...
        if next_step and len(next_step.strip()) > 10 and not is_continuation:
            self.todo_list = next_step
            return self.run_relay(f"繼續執行：{next_step}", is_continuation=True)
